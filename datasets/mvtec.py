...
class MVTecFSCopyPasteDataset(Dataset):
    ...
    def __init__(self, c, is_train=True):
        ...
        # prioritized anomaly types weights, passed as "type:mult" comma separated
        self.prioritized_weights = {}
        if hasattr(c, 'prioritized_ano_types') and c.prioritized_ano_types:
            try:
                for item in c.prioritized_ano_types.split(','):
                    if ':' in item:
                        t, m = item.split(':')
                        self.prioritized_weights[t.strip()] = float(m)
            except Exception:
                self.prioritized_weights = {}
        self.in_fg_region = c.in_fg_region
        # stronger augmentation parameters
        self.strong_ops = getattr(c, 'strong_ops', 6)
        self.use_seamless_clone = getattr(c, 'use_seamless_clone', False)
        self.placement_attempts = getattr(c, 'placement_attempts', 5)
        # load dataset (also returns anomaly types)
        self.n_imgs, self.n_labels, self.n_masks, self.a_imgs, self.a_labels, self.a_masks, self.a_types = self.load_dataset_folder()
        # repeat arrays according to repeat_num
        ...
        self.a_types = self.a_types * self.repeat_num
        ...
        # add stronger augmenters that can be used when prioritizing certain subtypes
        self.strong_augmentors = [
            A.ElasticTransform(alpha=120, sigma=6, alpha_affine=6, p=0.5),
            A.IAAPerspective(scale=(0.05, 0.2), p=0.5),
            A.GridDistortion(num_steps=5, distort_limit=0.3, p=0.5),
            A.CoarseDropout(max_holes=8, max_height=32, max_width=32, p=0.4),
            A.RandomResizedCrop(height=self.cropsize[0], width=self.cropsize[1], scale=(0.6, 1.0), p=0.4),
    ...
    def __getitem__(self, idx):
        if idx >= len(self.n_imgs):  # anomaly samples
            ...
            if idx >= len(self.n_imgs) + self.anomaly_nums * self.reuse_times:
                ...
                ano_type = None
                try:
                    ano_type = self.a_types[idx_]
                except Exception:
                    ano_type = None
                img, mask = self.copy_paste(img, mask, ano_type=ano_type)
    ...
    def randAugmenter_n(self, n_ops=3):
        # choose n_ops distinct augmentors (clamped to available)
        # allow selecting from expanded pool (augmentors + strong_augmentors)
        pool = self.augmentors + self.strong_augmentors
        n_ops = min(n_ops, len(pool))
        aug_ind = np.random.choice(np.arange(len(pool)), n_ops, replace=False)
        aug_list = [pool[i] for i in aug_ind]
        aug = A.Compose(aug_list)
        return aug

    def copy_paste(self, img, mask, ano_type=None):
        n_idx = np.random.randint(len(self.n_imgs))  # get a random normal sample
        # choose stronger augmenter for prioritized anomaly types
        if ano_type is not None and ano_type in self.prioritized_weights:
            aug = self.randAugmenter_n(n_ops=self.strong_ops)
        else:
        ...
        # ensure aug_image/aug_mask match the target normal image size
        try:
            if aug_image.shape[0] != img_height or aug_image.shape[1] != img_width:
                aug_image = cv2.resize(aug_image, (img_width, img_height), interpolation=cv2.INTER_LINEAR)
            # aug_mask may be single channel
            if aug_mask.shape[0] != img_height or aug_mask.shape[1] != img_width:
                aug_mask = cv2.resize(aug_mask, (img_width, img_height), interpolation=cv2.INTER_NEAREST)
        except Exception:
            pass
        ...
        if self.in_fg_region:
            ...
            if (np.sum(intersect_mask) > int(2 / 3 * np.sum(aug_mask == 255))):
                ...
                # paste; optionally use seamless clone for more realistic blending
                if self.use_seamless_clone:
                    try:
                        ys, xs = np.where(aug_mask == 255)
                        if ys.size > 0:
                            center = (int(xs.mean()), int(ys.mean()))
                            src = aug_image.astype(np.uint8)
                            dst = n_image.astype(np.uint8)
                            mask_clone = (aug_mask == 255).astype(np.uint8) * 255
                            n_image = cv2.seamlessClone(src, dst, mask_clone, center, cv2.MIXED_CLONE)
                            return n_image, aug_mask
                    except Exception:
                        # fallback to simple paste on any error
                ...
                if len(widths) == 0 or len(heights) == 0:  # no contours
                    ...
                    for i in range(len(contours)):
                        ...
                        # try multiple attempts to find a suitable foreground location
                        success = False
                        attempts = 0
                        delta_x = delta_y = 0
                        while attempts < self.placement_attempts and not success:
                            idx = np.random.choice(np.arange(xx_yy_fg.shape[0]), 1)
                            rand_xy = xx_yy_fg[idx]
                            delta_x_tmp, delta_y_tmp = center_xs[i] - rand_xy[0, 0], center_ys[i] - rand_xy[0, 1]
                            ...
                            xx_ano_shifted = xx_ano - delta_x_tmp
                            yy_ano_shifted = yy_ano - delta_y_tmp
                            ...
                            # success if not too many outer points
                            if xx_ano.size > 0:
                                frac_out = np.sum(outer_points) / float(xx_ano.size)
                            else:
                                frac_out = 1.0
                            if frac_out < 0.15:
                                success = True
                                delta_x, delta_y = delta_x_tmp, delta_y_tmp
                                break
                            attempts += 1
                        # if not success, fall back to last tried deltas
                        if not success:
                            try:
                                rand_xy = xx_yy_fg[np.random.choice(np.arange(xx_yy_fg.shape[0]), 1)]
                        ...
                        # shift the original mask into fg region using delta_x/delta_y
    ...
    def load_dataset_folder(self):
        ...
        # original behavior: allocate base per-type and scale by prioritized weight (may increase total)
        num_ano_types = len(ano_types) - 1
        anomaly_nums_per_type = self.anomaly_nums // num_ano_types if num_ano_types > 0 else self.anomaly_nums
        a_types = []
        for type_ in ano_types:
            img_type_dir = os.path.join(img_dir, type_)
            if not os.path.isdir(img_type_dir):
            ...
            if type_ == 'good':
                continue
            # randomly choose some anomaly images, possibly upweight certain subtypes
            random.shuffle(img_fpath_list)
            weight = self.prioritized_weights.get(type_, 1.0)
            select_n = int(round(anomaly_nums_per_type * weight))
            select_n = max(1, min(select_n, len(img_fpath_list)))
            selected = img_fpath_list[:select_n]
            a_img_paths.extend(selected)
            a_labels.extend([1] * len(selected))

            gt_type_dir = os.path.join(gt_dir, type_)
            ano_img_fname_list = [os.path.splitext(os.path.basename(f))[0] for f in selected]
            gt_fpath_list = [os.path.join(gt_type_dir, img_fname + '_mask.png')
                             for img_fname in ano_img_fname_list]
            a_mask_paths.extend(gt_fpath_list)
            a_types.extend([type_] * len(selected))
        ...
        return n_img_paths, n_labels, n_mask_paths, a_img_paths, a_labels, a_mask_paths, a_types
...
class MVTecPseudoDataset(Dataset):
    def __init__(self, c, is_train=True):
        ...
        self.repeat_num = 10
        self.reuse_times = 5
        # choose pseudo anomaly type: 'nsa' or 'perlin'
        self.ano_type = getattr(c, 'pseudo_type', 'nsa')
        ...
        # basic validation: perlin mode requires anomaly_source_path
        if self.ano_type == 'perlin' and (not c.anomaly_source_path or len(self.anomaly_source_paths) == 0):
            raise ValueError("Perlin pseudo anomalies require '--anomaly_source_path' pointing to a texture dataset (e.g., DTD). No images found.")
...
class MVTecAnomalyDataset(Dataset):
    ...
    def __len__(self):
        # combine: all of pseudo_dataset (normals + pseudo anomalies)
        # plus only the anomalies from copy_paste_dataset (exclude its normals)
        return len(self.pseudo_dataset) + (len(self.copy_paste_dataset) - len(self.copy_paste_dataset.n_imgs))

    def __getitem__(self, idx):
        pseudo_len = len(self.pseudo_dataset)
        if idx < pseudo_len:
            return self.pseudo_dataset.__getitem__(idx)
        # Map the remaining range to the anomaly slice of copy_paste_dataset
        cp_normal_len = len(self.copy_paste_dataset.n_imgs)
        cp_idx = cp_normal_len + (idx - pseudo_len)
        return self.copy_paste_dataset.__getitem__(cp_idx)
